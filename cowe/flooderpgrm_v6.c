...
//#ifndef ec_cmp
  #define ec_cmp
//#endif
//#ifndef ec_neg1 ec_cmp
//  #define ec_neg1 ec_cmp
//#endif
//#ifndef ec_cmp
...
flooder ParseStartFile() 
{
  ...
  if (UseFileDir)
  {
    ...
    {
      if (chdir(FILE_DIR) != 0)
        perror("Error changing to 'start' file directory\n");
    }
  }
  else
  {
...
int CreateChild()
{
  ...
  printf("In [CreateChild] - check for NULL flooder...\n");
  printf("In [CreateChild] - size of flooder: %d\n", sizeof(FLOODER));
  DisplayFlooder(f);

  /*
  if ((strcmp(FLOODER.sipaddr, "")) == 0)
  {
    UseFileDir = 1;

    printf("In [CreateChild] - no flooder found - re-parsing 'start' file...\n");

    // Flooder value has been lost - so read from saved file and re-parse
    FLOODER = ParseStartFile(); 
    UseFileDir = 0;
  }
  */

  printf("In [CreateChild] - display flooder passed in...\n");
  DisplayFlooder(f);
  ...
  switch(pid)
  {
    ...
    case 0:
      ...
      //execlp("/usr/sbin/packit", "packit", "-m", "inject", "-t", "UDP", "-s", "10.10.10.208", "-d", "10.10.10.9", "-S", f.sportno , "-D", f.dportno, "-c", "0", "-b", "0", "-p", "'0x 68 76 6D 72 20 74 65 73 74 20 75 70 64 20 70 61 63 6B 65 74 73'", (char *) NULL);
      //execlp("/usr/sbin/packit", "packit", "-m", "inject", "-t", "UDP", "-s", "10.10.10.208", "-d", "10.10.10.9", "-S", f.sportno , "-D", f.dportno, "-c", "0", "-b", "0", "-p", "'0x 68 76 6D 72 20 74 65 73 74 20 75 70 64 20 70 61 63 6B 65 74 73'", (char *) NULL);
     // execlp("/usr/sbin/packit", "packit", "-m", "inject", "-t", "UDP", "-s", FLOODER.sipaddr, "-d", FLOODER.dipaddr, "-S", FLOODER.sportno , "-D", FLOODER.dportno, "-c", "0", "-b", "0", "-p", "'0x 68 76 6D 72 20 74 65 73 74 20 75 70 64 20 70 61 63 6B 65 74 73'", (char *) NULL);
...
void Timer(int interval_time, int timer_time, flooder f)
{
  ...
  while (1)
  {
    ...
    // Verify flooder object 
    printf("In [Timer] - check FLOODER value...\n");
  ...
  {
  if (IsFlooding)
  //if (IsRunning && IsFlooding)
  //if (IsRunning && (IsFlooding == 1))
  {
    // End of running interval - stop flooding
    printf("In [Timer] - (flooding) - end of flooding interval\n");
    printf("In [Timer] - stop flooding (killing child pid %d...)\n", getpid());
  ...
  //else if (IsRunning && !IsFlooding)
  //else if (IsRunning && (IsFlooding == 0))
  else
  {
    // End of not-running interval - start flooding
    printf("In [Timer] - (not flooding) - end of not-flooding interval\n");
    printf("In [Timer] - start flooding (starting child pid...)\n");
    //if (CreateChild(FLOODER) == 0)
    if (CreateChild() == 0)
    {
      printf("Child pid successfully started (pid %d)\n", getpid());
      printf("IsFlooding = %s\n", IsFlooding == 1 ? "true" : "false");
      //IsFlooding = true;
      IsFlooding = 1;
    }
    else
      printf("In [Timer] - error creating child pid\n");
  }
  }
  */
    break;
...
void Handler(int signum)
{
  ...
  //if (IsRunning && (IsFlooding == 1))
  {
    ...
    //printf("In [Handler] - stop flooding (killing child pid %d...)\n", getpid());
    //printf("In [Handler] - stop flooding (killing child pid [current_pid] %d...)\n", current_pid);
    printf("In [Handler] - stop flooding (killing [current_pid]...)\n");
    //if (KillChildPid(getpid()) == 0)
    //if (KillChildPid(current_pid) == 0)
    //int result = KillChildPid(current_pid);
    result = KillCurrentPid();
    if (result == 0)
    {
      //printf("In [Handler] - child pid %d successfully killed\n", current_pid);
    ...
    {
      //printf("In [Handler] - error killing child pid: %d\n", result);
  ...
  //printf("In [Handler] - calling CreateChild()...\n");
  //if (IsRunning) { CreateChild(); }
      ...
      switch(GetFile())
      {
        case 1:
          ...
          */
          ...
          //FLOODER = f; 
          //printf("In [Main] - check flooder source IP address: %s)\n", FLOODER.sipaddr);
          //printf("In [Main] - check FLOODER after assignment from 'f'...\n");
